...
standard_translation = Korean2Dialect('standard', beam_search=False, k=0)  # 전라 번역 클래스 선언
...
standard_speech = Text2Speech('표준')  # 표준어 음성합성 클래스 선언
...
def tts_inference(gender, model_type, korean):
    ...
    elif model_type == 3:  # 표준어
        korean2dialect = standard_translation
        text2speech = standard_speech
    else:
        print(model_type)
...
def ml_inference():
    ...
    model_type = int(request.form['model'])  # [0, 1, 2, 3] = ['제주도', '경상도', '전라도','표준어]
...
def api_inference():
    ...
    model_type = int(data['model'])  # [0, 1, 2, 3] = ['제주도', '경상도', '전라도','표준어]
